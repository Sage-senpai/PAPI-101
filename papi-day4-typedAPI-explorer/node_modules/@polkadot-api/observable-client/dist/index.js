...
const getWithRecovery = () => {
  ...
  const process = (data) => {
    ...
    const subscription = source$.subscribe({
      ...
      error(e) {
        (isOperationLimit = e instanceof Error && e.name === substrateClient.OperationLimitError.errorName) ? addTask(data, true) : observer.error(e);
      },
      complete() {
        observer.complete();
      }
...
const withOperationInaccessibleRetry = (source$) => {
  const result = source$.pipe(
    rxjs.catchError(
      (e) => e instanceof Error && e.name === substrateClient.OperationInaccessibleError.errorName ? rxjs.timer(750).pipe(rxjs.concatMap(() => result)) : rxjs.throwError(() => e)
    )
  );
  return result;
};
...
function withStopRecovery(pinned$, fn, label) {
  return (hash, ...args) => {
    ...
    const inner$ = fn(hash, ...args).pipe(
      rxjs.map((value) => ({ type: "val", value })),
      rxjs.endWith({ type: "end" })
    );
    const waitForIt$ = pinned$.pipe(
      rxjs.takeWhile((x) => !x.blocks.has(hash)),
      rxjs.map((x) => {
    ...
    return pinned$.pipe(
      rxjs.map((x) => x.state.type === PinnedBlockState.Ready),
      rxjs.distinctUntilChanged(),
      rxjs.switchMap((isReady) => {
        if (!isReady) return rxjs.NEVER;
        block || (block = pinned$.state.blocks.get(hash));
        return block ? rxjs.concat(waitForIt$, inner$) : rxjs.throwError(getNonPinnedError);
      }),
      rxjs.takeWhile((x) => x.type === "val"),
      rxjs.map((x) => x.value)
    );
  };
}
  ...
  const getRuntimeChanges = (blocks, firstId, lastId) => {
    ...
    return getCodeHash(blocks[middleIdx]).pipe(
      rxjs.mergeMap((id) => {
        ...
        return middle.id === lastId.id ? getRuntimeChanges(blocks, firstId, middle) : rxjs.combineLatest([
          getRuntimeChanges(blocks, firstId, middle),
          getRuntimeChanges(blocks, middle, lastId)
        ]).pipe(rxjs.map(([left, [_SKIP, ...right]]) => [...left, ...right]));
      })
    );
  };
  return (blocks) => {
    ...
    if (blocks.length === 1)
      return getCodeHash(initialBlock).pipe(rxjs.map((x) => [[initialBlock, x]]));
    const lastIdx = blocks.length - 1;
    return rxjs.combineLatest([initialBlock, blocks[lastIdx]].map(getCodeHash)).pipe(
      rxjs.mergeMap(
        ([firstId, lastId]) => firstId === lastId ? rxjs.of([[initialBlock, firstId]]) : getRuntimeChanges(
          blocks,
          { idx: 0, id: firstId },
...
const withEnhancedFollow = (getFollower, getCodeHash, reset) => {
  ...
  const getRawHeader = (blockHash) => rxjs.defer(() => getFollower().header(blockHash));
  ...
  const getInnerObservables = (event) => {
    if (event.type === "initialized") {
      ...
      return rxjs.combineLatest([
        ...
        rxjs.map(([rawHeader, changes]) => {
          if (!hasher$.closed) {
            hasher$.next(getHasherFromHeader(rawHeader, blockHash));
            hasher$.complete();
          }
          const { number, parentHash, digests } = substrateBindings.blockHeader.dec(rawHeader);
          return {
            type: "initialized",
  ...
  const enhancer = (base) => new rxjs.Observable(
    (observer) => {
      let pending = null;
      let inner;
  ...
  return {
    getHeader: (blockHash) => getRawHeader(blockHash).pipe(rxjs.map(substrateBindings.blockHeader[1])),
    ...
    const setFollower = () => {
      follower = chainHead(
        ...
        (e) => {
          follower = null;
          if (isDone) return;
          if (e instanceof Error && e.name === substrateClient.StopError.errorName) {
            setFollower();
  ...
  const result = (...args) => {
    ...
    return hash ? fn(hash, ...args).pipe(
      rxjs.catchError((e) => {
        ...
        if (e instanceof Error && e.name === substrateClient.OperationInaccessibleError.errorName)
  ...
  return (codeHash, initialBlock) => {
    ...
    const runtimeContext$ = getMetadata$(
      ...
      getRawMetadata$(enhancer(call$)).pipe(
        rxjs.catchError((err) => err === done ? rxjs.EMPTY : rxjs.throwError(() => err))
      )
    ).pipe(
      rxjs.map(
...
const getPinnedBlocks$ = (follow$, call$, getCachedMetadata$, setCachedMetadata, blockUsage$, newBlocks$, onUnpin, deleteFromCache) => {
  ...
  const _pinnedBlocks$ = rxjs.merge(
    blockUsage$,
    cleanupEvt$,
    follow$
  ).pipe(
...
const getTrackTx = (blocks$, newBlocks$, getBody, getIsValid, getEvents, hodl) => {
  ...
  const hodl$ = new rxjs.Observable((observer) => {
  ...
  const whileBlockActive = (hash) => rxjs.takeUntil(
    blocks$.pipe(rxjs.filter(({ blocks }) => blocks.get(hash)?.pruned ?? true))
  );
  ...
  const findInBranch = (hash, tx, alreadyPresent) => analyzeBlock(hash, tx, alreadyPresent.has(hash)).pipe(
    rxjs.mergeMap((analyzed) => {
      ...
      return isSettled ? rxjs.of(analyzed) : blocks$.pipe(
...
const getChainHead$ = (chainHead, getCachedMetadata, setCachedMetadata) => {
  ...
  const unpin = (hashes) => getFollower().unpin(hashes).catch((e) => {
    if (e instanceof Error && e.name === substrateClient.DisjointError.errorName) return;
    throw e;
  });
  ...
  const readyBlocks$ = Object.assign(
    pinnedBlocks$.pipe(rxjs.filter((x) => x.state.type === PinnedBlockState.Ready)),
  ...
  const getRuntime$ = (codeHash) => pinnedBlocks$.state.runtimes[codeHash]?.runtime ?? rxjs.of(null);
  ...
  const newBlocks$ = _newBlocks$.pipe(rxjs.takeWhile(Boolean));
  ...
  return {
    ...
    best$,
    bestBlocks$,
    newBlocks$: rxjs.concat(
      readyBlocks$.pipe(
        rxjs.take(1),
        rxjs.mergeMap(({ blocks, finalized }) => {
